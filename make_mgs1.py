...
def format(line : str):
    ...
    for type, token in tokens:
        match type:
            case 'TAG':
                ...
                match token:
                    case 'space=102px':
                        stack.append(f'<div class="show0"><p class="speaker">')
                        istack.append('</p></div>')
                    case 'size=24':
                        stack.append(f'<div class="show1"><b>')
                        istack.append('</b></div>')
                    case 'width=613':
                        stack.append(f'<div class="show3">')
                        istack.append('</div>')
                    case 'br':
                        if (stack and stack[-1] == '<br>'):
                            continue
                        stack.append(f'<br>')
                        istack.append('</br>')
            case 'TEXT':
    ...
    return res.getvalue()
def __main__():
    parser = ArgumentParser(description="METAL GEAR SOLID MASTER COLLECTION VOL. 1 - MGS1 Screenplay Compilation Tool")
    ...
    with open(output, 'w', encoding='utf-8') as f:
        ...
        with open(args.head, 'r', encoding='utf-8') as head:
            f.write(head.read())        
        keys = ['TextAreaA0','TextAreaA1','TextAreaA2','TextAreaA3','TextAreaB0','TextAreaB1','TextAreaB2','TextAreaB3']
        for item in data:
            for key in keys:
